...
export class HttpError<T = unknown> extends Error { // custom error để gắn status + data
    status: number; // HTTP status trả về từ BE
    data?: T; // dữ liệu lỗi (nếu BE trả JSON)

    constructor(status: number, message: string, data?: T) { // khởi tạo lỗi với status + message + data
...
type ApiOptions = Omit<RequestInit, "body"> & { // option mở rộng từ fetch option
...
function buildUrl(baseURL: string, path: string, query?: Query) { // build URL đầy đủ
    const url = new URL(path, baseURL); // tạo URL object từ base + path
    if (query) { // nếu có query
        for (const [k, v] of Object.entries(query)) { // duyệt từng key/value
            if (v) { // bỏ qua giá trị null/undefined/false/0
...
function readSetCookies(res: Response): string[] { // đọc Set-Cookie từ response
    const anyHeaders = res.headers as unknown as { getSetCookie?: () => string[] }; // ép kiểu để đọc getSetCookie nếu có
    const fromGetSetCookie = typeof anyHeaders.getSetCookie === "function" ? anyHeaders.getSetCookie() : undefined; // gọi getSetCookie nếu tồn tại
    if (fromGetSetCookie && fromGetSetCookie.length > 0) { // nếu có cookie dạng mảng
...
async function requestRaw<T = any>( // hàm fetch đầy đủ metadata
...
): Promise<ApiRawResponse<T>> { // trả về ApiRawResponse
    const { // bóc tách option
    ...
    if (guestSessionId && !header.has("cookie")) { // chỉ set nếu có guestSessionId và chưa có cookie header
        header.set('cookie', `guestSessionId=${cookieStore.get('guestSessionId')?.value || ''}`); // gắn cookie để BE đọc
    }

    // Gắn accessToken vào Authorization nếu chưa có
    if (accessToken && !header.has("authorization")) { // nếu có token và chưa set Authorization
        header.set("authorization", `Bearer ${accessToken}`); // set header Authorization
    }

    // Gắn ngôn ngữ app để BE trả nội dung đúng ngôn ngữ
    if (language) { // nếu có language
        header.set("x-app-lang", language); // set custom header
    }

    let res: Response; // biến lưu response
    try {
        res = await fetch(url, { // gọi fetch đến BE
    ...
    if (!res.ok) { // nếu status không phải 2xx
        // Nếu unauthorized thì throw lỗi riêng để caller xử lý
        if (res.status === HttpStatusCode.Unauthorized) { // kiểm tra 401
            throw new HttpError(res.status, "Unauthorized", response); // ném lỗi 401 riêng
        }
        const msg =
            (isJson && response && (response.message)) ? (response.message || 'InternalServerError') : `HTTP ${res.status ?? 505}`; // ưu tiên message từ BE
...
export const fetchApi = (defaults: Pick<ApiOptions, "baseURL" | "headers"> = {}) => { // factory tạo api client
    ...
    return { // trả ra các method HTTP
        get<T = any>(path: string, opt?: ApiOptions) { // GET wrapper
            return request<T>("GET", path, withDefaults(opt)); // gọi request đơn giản
        },
        post<T = any>(path: string, body?: unknown, opt?: ApiOptions) { // POST wrapper
            return request<T>("POST", path, withDefaults({ ...opt, body })); // gộp body vào option
        },
        put<T = any>(path: string, body?: unknown, opt?: ApiOptions) { // PUT wrapper
            return request<T>("PUT", path, withDefaults({ ...opt, body })); // gộp body vào option
        },
        patch<T = any>(path: string, body?: unknown, opt?: ApiOptions) { // PATCH wrapper
...
export const api = fetchApi({ // instance api dùng chung toàn app
