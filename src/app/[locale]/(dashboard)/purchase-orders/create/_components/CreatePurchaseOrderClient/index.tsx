  ...
  return (
    <div className="min-w-0">
      <div className="topbar">
        ...
        <div>
          <div className="text-[15px] font-semibold text-ink">Tạo Đơn Nhập Hàng Mới</div>
          <div className="text-[11px] text-ink-3">Điền thông tin và thêm sản phẩm để tạo đơn nhập hàng</div>
        </div>
        <div className="ml-auto flex items-center gap-2">
          <button className="btn-soft rounded-lg px-3.5 py-2 text-[12.5px]">Lưu Nháp</button>
          <button 
        ...
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
          <div className="space-y-4 lg:col-span-2">
            ...
            <div className="card p-5">
              <h4 className="display text-[17px] font-semibold text-ink">Thông tin chung</h4>
              <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  {form.formState.errors.supplierId && (
                    <p className="mt-1 text-xs text-accent">{form.formState.errors.supplierId.message}</p>
                  )}
                </div>
                <div>
            ...
            <div className="card overflow-hidden">
              <div className="flex items-center justify-between px-5 py-4">
                <h4 className="display text-[17px] font-semibold text-ink">Sản phẩm trong đơn</h4>
                {purchaseItems.length > 0 && (
                  <span className="bdg bdg-blue">{purchaseItems.length} sản phẩm</span>
                )}
              </div>
                  ...
                  <table className="tbl w-full">
                    ...
                    <tbody>
                      {purchaseItems.map((item) => (
                        <tr key={item.id}>
                          <td className="font-semibold text-ink max-w-[200px] truncate" title={item.bookVariantName}>
            ...
            <div className="card p-5">
              ...
              <div className="mt-4 space-y-2.5 text-[13px]">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium text-ink">{purchaseItems.length} loại · {totalQty} items</span>
                </div>
                ...
                <div className="flex justify-between">
                  <span className="text-ink-2">Thuế</span>
                  <span className="font-medium text-ink">{taxAmount.toLocaleString()} ₫</span>
                </div>
                <div className="hairline my-2"></div>
                <div className="flex items-center justify-between text-[15px]">
                  <span className="font-semibold text-ink">Tổng thanh toán</span>
                  <span className="display font-semibold text-accent">{grandTotal.toLocaleString()} ₫</span>
                </div>
              </div>
              <button 
